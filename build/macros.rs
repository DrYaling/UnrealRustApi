...
macro_rules! log_warn {
    ($($arg:tt)+) => ({
        warn!("{} at file {}, line {}", format_args!($($arg)+), file!(),line!())
    })
}
///wraper of text!()
/// 
...
/// ```
/// let state_id = 1;
/// return battle_err!("fail to start state {}", state_id);
/// ```
#[macro_export]
macro_rules! battle_err {
    ($($arg:tt)+) => ({
        let ret = format!("{} at file {}, line {}", format_args!($($arg)+), file!(),line!());
        $crate::battle::error::battle_result(ret)
    })
}
/// destruct structure
/// ```
